...
impl CounterScopes {
    pub(crate) fn enter(&mut self, depth: usize) {
        self.entries.retain(|entry| entry.depth <= depth);
    }

    pub(crate) fn run(&mut self, depth: usize, ops: &CounterOps) {
    ...
    pub(crate) fn value(&self, name: &str) -> i64 {
        self.entries
            .iter()
            .rev()
            .find(|entry| entry.name == name)
    ...
    pub(crate) fn values(&self, name: &str) -> Vec<i64> {
        self.entries
            .iter()
...
pub(crate) struct CounterOps {
    pub(crate) reset: Vec<(String, i64)>,
    pub(crate) increment: Vec<(String, i64)>,
    pub(crate) set: Vec<(String, i64)>,
}
...
pub(crate) fn merge_counter_ops(
...
pub(crate) fn ua_counter_ops(document: &Document, id: NodeId, style: ComputedStyle) -> CounterOps {
